...
gcc-4.8)
    ...
    echo $(brew --prefix llvm@3.9) >> travis_env_paths
    ;;
gcc-4.9)
    install_brew_package gcc@4.9
    echo $(brew --prefix gcc@4.9) >> travis_env_paths
    ;;
gcc-5)
    install_brew_package gcc@5 
    echo $(brew --prefix gcc@5) >> travis_env_paths
    ;;
gcc-6)         
    install_brew_package gcc@6 
    echo $(brew --prefix gcc@6) >> travis_env_paths
    ;;
clang-default) ;;
clang-3.7)     
    install_brew_package llvm@3.7 --with-clang --with-libcxx
    echo $(brew --prefix llvm@3.7) >> travis_env_paths
    ;;
clang-3.8)     
    install_brew_package llvm@3.8 --with-clang --with-libcxx
    echo $(brew --prefix llvm@3.78) >> travis_env_paths
    ;;
clang-3.9)     
...
clang-4.0)     
    ...
    echo $(brew --prefix llvm) >> travis_env_paths
    ;;
